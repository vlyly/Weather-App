...
function getWeatherForecast(latitude, longitude) {
  fetch(
    ...
    .then(function (json) {
      ...
      for (let i = 0; i < 16; i++) {
        const forecastHour = document.getElementById(`item${i}_hour`);
        const forecastIcon = document.getElementById(`item${i}_icon`);
        const forecastMain = document.getElementById(`item${i}_main`);
        const forecastTemp = document.getElementById(`item${i}_temp`);
        ...
        forecastHour.innerText = `${hour}시`;
        forecastIcon.src = `icon/${forecast_weather_icon_code}.png`;
        forecastMain.innerText = forecast_weather_main_data;
        forecastTemp.innerText = `${forecast_temperature_data}`;
      }
    });
}
